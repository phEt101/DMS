...
export async function logActivity({ userId = null, action, entityType, entityId = null, details = null }: ActivityInput) {
  await db.execute(
...
export async function findRecent(limit = 100) {
  const [rows] = await db.query(
    `SELECT a.id, a.action, a.entity_type AS entityType, a.entity_id AS entityId,
            a.details, a.created_at AS createdAt, u.name AS userName
